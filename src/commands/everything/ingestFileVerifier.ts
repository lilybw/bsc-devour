...
import {
    ...
    INGEST_FILE_SINGLE_ASSET_FIELD_TYPEDECL,
    INGEST_FILE_SINGLE_ASSET_TYPEDECL,
    IngestFileAssetType,
...
import { DBDSN_TYPEDECL, INGEST_FILE_COLLECTION_ENTRY_TYPEDECL, TRANSFORM_DTO_TYPEDECL, type DBDSN, type TransformDTO } from '../../ts/types';
...
export const assureUniformTransform = (maybeTransform: string | TransformDTO): ResErr<TransformDTO> => {
    if (typeof maybeTransform === 'string') {
        ...
        if (typeError !== null) {
            return {
                result: null,
                error: 'Transform field does not conform to type: ' + typeError,
            };
        }
        ...
        return {
            result: null,
            error: 'Transform field is not compact CLI notation (string) or an object.',
        };
    }
};
...
export const validateSingleAssetEntry = (asset: IngestFileSingleAsset, entryNum: number, context?: ApplicationContext): string | null => {
    ...
    if (!asset.single.alias || asset.single.alias === null) {
        ...
        context?.logger.log(
            '[ingest] No alias provided for single asset nr: ' + entryNum + ', using source name as alias: ' + asset.single.alias,
            LogLevel.WARNING,
        );
    }
...
export const assureUniformDSN = (dsn: string | DBDSN): ResErr<DBDSN> => {
    if (typeof dsn === 'string') {
        ...
        if (typeError !== null) {
            return {
                result: null,
                error: 'DSN object does not conform to expected type:\n\t' + typeError,
            };
        }
        ...
        return {
            result: null,
            error: 'DSN field is not compact CLI notation ("host port, username password, dbName, sslMode") or an object.',
        };
    }
};
...
export const verifySubFileIDAssignments = (
    subFile: SettingsSubFile,
    script: AutoIngestSubScript,
